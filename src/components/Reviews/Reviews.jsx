...
export default function Reviews({ idPsychologist }) {
  ...
  //me traigo la info del profesional para conocer sus reviews previas
  ...
  const validate = (input_review) => {
    ...
    if (input_review.Rating === 0) {
      errors.Rating = "Debes asignarle estrellas al profesional";
    };
  ...
  return (
    ...
    <Stack align={'center'} >
      ...
      <Button onClick={onOpen} bg='#090964' color={'white'} >CALIFICA A TU PROFESIONAL</Button>
      <Modal
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalHeader margin={'auto'} fontSize='2xl' color={'#090964'}>CALIFICA A TU PROFESIONAL</ModalHeader>
          <ModalCloseButton />
          ...
          <ModalBody pb={5}>
            <FormControl >
              <textarea type="text" className="input_calificacion" value={input_review.Content} name='Content' onChange={(e) => handleInputChange(e)} placeholder="Deja un reseña sobre tu profesional" />
              {errorsReview.Rating && <Text fontWeight={'semibold'} color={'#090964'} >{`> ${errorsReview.Rating}`}</Text>}
              {errorsReview.Content && <Text fontWeight={'semibold'} color={'#090964'} >{`> ${errorsReview.Content}`}</Text>}
            </FormControl>
          </ModalBody>

          <ModalFooter>
            <Button type='submit' bg={'#090964'} color='white' variant='outline' _hover={[{ color: '#090964' }, { bg: 'white' }]} disabled={isSubmit} onClick={(e) => { handleSubmit(e) }} colorScheme='blue' mr={1}>
              Enviar
            </Button>
          </ModalFooter>
