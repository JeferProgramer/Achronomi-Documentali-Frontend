   ...
   return (
      <VStack w={'100%'} alignItems={'space-between'}>
         ...
         <Grid templateColumns='repeat(7, 1fr)' templateRows='repeat(7, 1fr)' gap={3}>
            {d.map((d) => (
               <GridItem colSpan={1} w='100%' h='10' key={d.toString()}>
                  <Badge w='100%' colorScheme='#090964' fontSize='xl' fontWeight='bold'>{d}
                  </Badge>
               </GridItem>
            ))}
            {days.map((day, dayIdx) => {
               return (
                  <GridItem colStart={(dayIdx === 0 && colStartClasses[getDay(day)])} rowSpan={1} w='100%' h='10' key={day.toString()}>
                     {isEqual(day, today) ?
                        <Button disabled w='100%' colorScheme='#090964' bgColor={'gray.500'} color={'white'} _hover={{ bg: 'gray.200', color: 'black' }} _focus={{ bg: '#090964.300', color: 'white' }} _active={{ bg: '#090964.300', color: 'bl' }}>
                           <time dateTime={format(day, 'yyyy-MM-dd')}>
                              {format(day, 'd')}
                           </time>
                        </Button> : (available && available.includes(formatISO(new Date(day)).substring(0, 10))) ?
                           <Button onClick={() => handleDate(day)} w='100%' colorScheme='#090964' bgColor={'#090964'} color={'white'} _hover={{ bg: '#090964.400', color: 'white' }} _focus={{ bg: '#090964.300', color: 'white' }} _active={{ bg: '#090964.300', color: 'white' }}>
                              <time dateTime={format(day, 'yyyy-MM-dd')}>
                                 {format(day, 'd')}
                              </time>
                           </Button> :
                           <Button disabled w='100%' colorScheme='#090964' bgColor={'gray.500'} color={'white'} _hover={{ bg: 'gray.200', color: 'black' }}>
                              <time dateTime={format(day, 'yyyy-MM-dd')}>
                                 {format(day, 'd')}
                              </time>
                           </Button>
