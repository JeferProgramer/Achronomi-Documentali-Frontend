...
import { Stack, Text, Avatar, Button, Select } from '@chakra-ui/react';
            ...
            <div className='appointmentsContainer'>
              <Stack>
                ...
                <Stack pr='15%' pl='15%' pt='1em' pb='2em'>
                  ...
                  <Text fontWeight="semibold" fontSize="3xl" color="#090964" textAlign='left' mb='1em' >
                    Mis citas
                  </Text>
                  {
                    appointments.length !== 0
                      ? appointments.map((appo) => {
                                ...
                                tokenClient && appo.IdUserPsychologist
                                  ? (<Stack align='center' width='50%' borderRightWidth='0.1em' borderColor='#b7b7b7'>
                                    <Link to={`/detailPsychologist/${appo.IdUserPsychologist._id}`} cursor='pointer'>
                                      <Avatar size='xl' src={`${baseURL}/${appo.IdUserPsychologist.profileImage}`} />
                                ...
                                </Stack>
                                {
                                  (appointmentDate - todayDate > 0)
                                    ? <Stack direction='row'>
                                      {
                                        tokenClient
                                          ? <Button bg='white' colorScheme='#090964' variant='outline' onClick={() => handleChangeType(appo._id)}>
                                            Cambiar modalidad
                                          </Button>
                                          : null
                                      }
                                      <Button colorScheme='#090964' variant='outline' onClick={() => handleDeleteAppointment(appo._id, appo.IdSchedule)}>
                                        Cancelar cita
                                      </Button>
                                    </Stack>
                                    : <>
                                ...
                                changeTypeAlert
                                  ? <Stack className='changeType'>
                                    <Stack direction='column' bg='white' p='2em' align='center' borderRadius='1em' boxShadow={`0px 0px 10px 0px rgba(0,0,0,0.3)`}>
                                      ...
                                      <Stack direction='row'>
                                        <Button colorScheme='#090964' variant='solid' onClick={() => changeType()}>
                                          Aceptar
                                        </Button>
                                        <Button colorScheme='#090964' variant='outline' onClick={() => setChangeTypeAlert(false)}>
                                          Cancelar
                                        </Button>
                                      </Stack>
                                    </Stack>
                        ...
                        <Text bgGradient='linear(to-l, #319795, #090964)'
                          bgClip='text'
